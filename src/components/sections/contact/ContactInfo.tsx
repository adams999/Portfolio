...
export function ContactInfo({ contactInfo, className }: ContactInfoProps) {
  ...
  return (
    <div className={cn("bg-white dark:bg-white/5 rounded-2xl p-6 border border-gray-200 dark:border-white/10 shadow-sm", className)}>
      <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">{t('contactInformation')}</h3>
        ...
        {contactInfo.map((info) => {
          ...
          return (
            <div key={info.id} className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-500">{t(info.id as 'email' | 'whatsapp' | 'github' | 'linkedin')}</p>
                <Link
                  href={info.href}
                  {...linkProps}
